...
async function fetch_token(params: URLSearchParams): Promise<GithubAuthResponse> {
...
async function get_auth_token(res: Response, secret: string, exchange_function: (arg: string) => Promise<GithubAuthResponse>): Promise<void> {
  try {
    const github_response = await exchange_function(secret);

    console.log("Authorization successful!");

    res.cookie("refresh_token", github_response.refresh_token, cookie_settings(github_response.refresh_token_expires_in));
    res.status(200).json({ "user_token": github_response.access_token });
  } catch(error) {
    res.status(401).json({ error: `Could not get auth token. Error: ${error}` });
  }
}

app.post("/login", (req: Request, res: Response) => {
  if (!("refresh_token" in req.cookies)) return res.status(401).json({ error: "Invalid refresh token" });

  get_auth_token(res, req.cookies.refresh_token as string, exchange_refresh_token);
});
